...
type SalarySlip struct{
	...
	BasePay float64 `orm:"description(基本工资);digits(12);decimals(1)"`
	WorkingDay float64 `orm:"description(工作天数);digits(3);decimals(1)"`
	...
	PersonalPncomeTax float64 `orm:"description(个人税务);digits(6);decimals(2)"`
	Fine float64 `orm:"description(罚金);digits(6);decimals(2)"`
	NetSalary float64 `orm:"description(实发工资);digits(10);decimals(2)"`
	PayDate string `orm:"size(32);column(pay_date);description(工资月份)"`
	CreateTime time.Time `orm:"type(datetime);auto_now"`
}

func (t * SalarySlip)TableName() string{
