...
func (c *CarsApplyController) DoApply()  {
	...
	cars_date := models.Cars{Id:cars_id}

	o := orm.NewOrm()

	// 默认：ReturnStatus=0，AuditStatus=3，IsActive=1
	cars_apply := models.CarsApply{
		User:&user,
		Cars:&cars_date,
		Reason:reason,
...
func (c *CarsApplyController) DoAuditApply()  {
	...
	qs1 := o.QueryTable("sys_cars")
	qs1.Filter("CarsApply__Id",id).One(&car)
	if auditStatus == 1{
	qs1.Filter("id",car.Id).Update(orm.Params{
			"status": 2,
		})
	}else{
		qs1.Filter("id", car.Id).Update(orm.Params{
